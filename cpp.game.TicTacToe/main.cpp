// source: https://github.com/prabh124/TicTacToe
...
int main() {
    // Seed random time slots to allow for true randomization later on in the
    // program
    srand(time(NULL));
    ...
    int move;
    int totalMoves = 0;
    // Turns are currently left blank, they will be assigned to either "x" or
    // "o" depending on the turnCounter
    char playerTurn = ' ';
    char computerTurn = ' ';
    ...
    // While loop that continues until the max amount of turns have been
    // reached, or if someone has won
    while (winChecker(board) == 0 && totalMoves < 9) {
        ...
        if (turnCounter == 1) {
            ...
            // if the total moves mod 2 is 0, the player has the first turn and
            // becomes x, else the player is o
            if (!(totalMoves % 2)) {
                playerTurn = 'x';
            ...
            while ((moveIsValid(board, move) == false) || (move <= 0) ||
                   (move >= 10))

            {
                cout << "Invalid move! Please enter a number that has not been "
                        "chosen as of yet!  ";
                cin >> move;
            }
            // Set board at [move - 1] = either "x" or "o" depending on the
            // player turn
        ...
        // If the turnCounter is 2, it is the computers turn
            ...
            if (totalMoves == 0) {
                cout << "The computer is going first! Lets see what it does! "
                     << endl;
            }

            // if the total moves mod 2 is 0, the computer has the first turn
            // and becomes x, else the computer is o
            if (!(totalMoves % 2)) {
                computerTurn = 'x';
            }
        ...
        // Add 1 to totalMoves, so that the while loop will break if the game is
        // a tie
        totalMoves++;
    ...
    case 1:
        ...
        // If the computer did not go first and the winChecker returns 1, then
        // the player has won
        if (computerTurn != 'x')
    ...
    case 2:
        ...
        // If the computer did not go second and the winChecker returns 2, then
        // the player has won.
        if (computerTurn != 'o')
            cout << "You have won the game!" << endl;
...
bool moveIsValid(char array[], int move) {
    // If the inputted number has not already been taken in a previous turn,
    // return true.
