#include "../../Projects/webpp/webpp/middleware/middleware_concepts.hpp"
...
struct FlatImpl : FlatBase {
  int res = 1;
  NOINLINE int op1() override { return res++ + 1; }
  NOINLINE int op2() override { return res++ + 2; }
  NOINLINE int op3() override { return res++ + 3; }
};
...
struct DeepMIImpl : TemplatedMIImpl<MIBase1, MIBase2, MIBase3> {
  int res = 1;
  NOINLINE int op1() override { return res++ + 11; }
  NOINLINE int op2() override { return res++ + 22; }
  NOINLINE int op3() override { return res++ + 33; }
};
...
static void WebppMiddleware(benchmark::State &state) {
  ...
  for (auto _ : state) {
    root(on_middleware);
    benchmark::DoNotOptimize(one.res);
    root(on_middleware);
    benchmark::DoNotOptimize(one.res);
    root(on_middleware);
    benchmark::DoNotOptimize(one);
