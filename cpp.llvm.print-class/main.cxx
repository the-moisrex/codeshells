  ...
  void HandleTranslationUnit(ASTContext &Ctx) override {
    ...
    if (matches.empty()) {
      CI.getDiagnostics().Report(DiagnosticsEngine::Warning)
          << "print-class plugin: no class named '" << TargetName << "' found in TU";
      return;
    }
    ...
    for (auto *C : matches) {
      // Print the class declaration (this prints the declaration text as
      // it would appear in source for that declaration node).
      C->print(out, Policy);
      out << "\n";

      // Now find any method definitions that belong to this class but are
      // *out-of-line* (i.e. defined outside the class body). We iterate
      // the translation unit to find FunctionDecl/CXXMethodDecls that
      // have this class as their parent and are definitions.
        ...
        if (auto *FD = dyn_cast<FunctionDecl>(D)) {
          ...
          if (auto *MD = dyn_cast<CXXMethodDecl>(FD)) {
            if (MD->getParent() == C && MD->isThisDeclarationADefinition()) {
              // If the declaration is actually inside the class body (i.e.
              // the "in-class" definition), the MethodDecl *is* located
              // nested inside the CXXRecordDecl and will typically not
              // appear separately as a top-level TU decl; but many
              // out-of-line definitions *are* top-level FunctionDecls in
              // the TU, so this catches those.

              // Avoid printing the same method twice: skip if this method
              // is already defined inline inside the class (isOutOfLine
              // returns true for out-of-line definitions).
              if (!MD->isOutOfLine())
                continue;

              // Print the full function definition using the printing policy.
              MD->print(out, Policy);
              out << "\n";
            }
          }
...
public:
  ...
  bool ParseArgs(const CompilerInstance &CI,
    ...
    if (ClassName.empty()) {
      CI.getDiagnostics().Report(DiagnosticsEngine::Warning)
          << "print-class plugin: no class name provided; use -plugin-arg-print-class=class=Foo";
