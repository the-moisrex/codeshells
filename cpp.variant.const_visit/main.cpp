...
// from: https://stackoverflow.com/a/72319998/4987470
...
static void AutoVisit(benchmark::State& state) {
  SETUP_RANDOM_VARIANTS
...
static void ConstVisit(benchmark::State& state) {
  SETUP_RANDOM_VARIANTS

  for (auto _ : state) {
    auto res = std::visit([](const auto & v) { return v.get(); }, *package);
